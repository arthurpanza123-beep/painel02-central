...
export async function POST(request: Request) {
  ...
  const activation = body.activation || {}
    ...
    provider_code: String(test.provider_code || test.providerCode || activation.provider_code || activation.providerCode || body.context?.provider_code || body.context?.providerCode || ''),
    providerCode: String(test.providerCode || test.provider_code || activation.providerCode || activation.provider_code || body.context?.providerCode || body.context?.provider_code || ''),
    host: String(test.host || activation.host || body.context?.host || ''),
    dns: String(test.dns || activation.dns || activation.smart_tv_dns || body.context?.dns || ''),
